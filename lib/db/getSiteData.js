 ...
 * 获取公告（随机从所有公告中选择一条）
 */
async function getNotice(posts) {
  ...
  // 获取公告内容
  selectedPost.blockMap = await getPage(selectedPost.id, 'data-notice')
  return selectedPost
}
...
async function convertNotionToSiteDate(pageId, from, pageRecordMap) {
  ...
  // 获取所有符合条件的Notice文章
  const noticeList = collectionData.filter(post => {
    return (
      post &&
      post?.type &&
      post?.type === 'Notice' &&
  ...
  // 从所有公告中随机选择一条
  const notice = await getNotice(noticeList)
